...
const HelpingTools = ({ userId, onLogout }) => {
  ...
  const handleUpdateLink = async (id, newUrl, type) => {
    try {
      const toolData = {
        userId: userId,
        type: type.toLowerCase(),
        url: newUrl,
        description: `Updated ${type} Link`
      };
      
      await helpingToolsAPI.updateHelpingTool(id, toolData);
      setSuccess(`${type} link updated successfully!`);
      await fetchHelpingTools();
    } catch (error) {
